...
export class AuthService {
  ...
  async signIn(data: SignInDto) {
    try {
      ...
      const refresh_token = await this.jwtService.signAsync({ id: user.id }, { expiresIn: '7d' })
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const { password, ...userWithoutPassword } = user
      const payload = this.buildResponsePayload(userWithoutPassword, access_token, refresh_token)
  ...
  async resendVerifyToken(email: string) {
    try {
      ...
      const checkVerificationToken = await this.prisma.verificationToken.findFirst({
        where: { identifier: email },
      })

      if (!checkVerificationToken) throw new NotFoundException('Token not found')

      const generateToken = customAlphabet('123456789', 6)
      const token = generateToken(6)

      const newEmail = await this.prisma.verificationToken.update({
        where: { identifier: email },
        data: {
          token,
          expires: new Date(Date.now() + FIVE_MINUTES * 30),
  ...
  async changePassword(id: string, password: string, new_password: string) {
    const user = await this.prisma.user.findFirst({
      where: { id },
    })
  ...
  private async validateVerifyToken(token: string) {
    const checkToken = await this.prisma.verificationToken.findFirst({
      where: { token },
    })

    if (!checkToken) throw new HttpException('Invalid token', HttpStatus.BAD_REQUEST)
  ...
  async validateResetPasswordOtp(token: string) {
    const checkToken = await this.prisma.passwordReset.findFirst({
      where: { token },
    })
  ...
  private async verifyPassword(storedPassword: string, providedPassword: string) {
    const isValidPassword = await argon2.verify(storedPassword, providedPassword)

    if (!isValidPassword) throw new HttpException('Invalid password', HttpStatus.BAD_REQUEST)
